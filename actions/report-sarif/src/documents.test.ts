...
test('a misconfiguration is shaped like a posture finding, not a vulnerability', () => {
  ...
  const benchmark = document.rule.benchmark as Record<string, unknown>
  ...
  assert.equal(benchmark.id, 'opengrep')
  assert.equal(benchmark.name, 'Opengrep OSS')
  assert.equal(document.resource.name, '.github/workflows/publish.yml')
  assert.equal(document.resource.sub_type, 'yaml')
  assert.equal(document.resource.line, 22)
  assert.equal(document.resource.directory, '.github/workflows')
  assert.equal(document.resource.file, 'publish.yml')
  assert.equal(benchmark.rule_number, 'kestra-mutable-action-tag')
  assert.equal(document.rule.rationale, 'A tag can be repointed.')
