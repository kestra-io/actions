...
trace id, deterministic span ids), but each layer carries its own
`service.name` (plus a `telemetry.source` = `github-actions` | `gradle` | `junit`
span attribute) so a trace backend can group, color and filter the three layers
apart instead of bucketing every span under one service.
...
| `cgroup-metrics-enabled` | `true` | Run a background poller emitting this job's own `container.cpu.*`/`container.memory.*` cgroup metrics (including CPU throttling). Unlike the host-metrics `cpu`/`memory` scrapers above, which read `/proc` and report the whole **node** on Kubernetes runners, these reflect the cgroup limit actually enforced on this job. No-ops on hosts without a cgroup filesystem (macOS/Windows). Only takes effect when `host-metrics-enabled` is also true |
